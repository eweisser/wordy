...
# In version 8, I planned to get the program to keep track of how many zero-letters are in candidate words, and choose words that have two or three when possible.
# The last version, 9, was quite successful. It built off v8's successful targeting of words with letters already ruled out. Specifically, it will assign greater value to letters that are in words containing letters that are already ruled out (obviously, the ruled-out letters should not get points).
# This version, 10, will try to implement a mostly-new methodology, "two-point-oh", that divides the alphabet into mutually exclusive "containers" labelled with how many matches it has. These containers originate from a single guessed word.
...
def compTakesAGuess():
    ...
    # lexicon "maintenance" or "pruning"
    shouldBeDeletedFromLex.clear()  # we need to clear this list so that words that have already been deleted don't stay in it
    ...
    if currentUserFeedback == 0:    # The user said the computer's latest guess had 0 correct letters.
        ...
        if thisWordsLettersRuledOut == 3:
            for x in range(5):
                if currentCompGuess[x] not in lettersRuledOut and currentCompGuess[x] not in lettersRuledIn:
                    lettersRuledIn.append(currentCompGuess[x])

        if savedWords.get(currentCompGuess,0) < 1:
            print ("A new word with 2!")
