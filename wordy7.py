...
def userTakesAGuess():
    ...
    found = False
    for z in range(len(dictionary)):    # making sure the user's guess is an acceptable word to guess
        if currentUserGuess == dictionary[z]:
            found = True    # yes, the user's guess IS in the dictionary
    if found == False:
...
def compTakesAGuess():
    ...
    if random() < 0.0000001 and turnNumber > 9:     # A very small chance to pick a random word; basically a placeholder in case I want to increase the odds later
    ...
    else:   # pretty much always do this:
        candidateList = []
        ...
        for z in range(dictionarySectionSize): # don't look at every word in the dictionary, just get 1000
        ...
        if random() < turnNumber / 30:	# more likely to choose (M) maximum-scoring word as time goes by (by turn 30, 50%)
            currentCompGuess = candidateList[candidateMultiplyScoreList.index(max(candidateMultiplyScoreList))]
            print ("!!!!!!!!!!!!!!!turnNumber/60 exceeded!")
    ...
    dictionary.remove(currentCompGuess)
    ...
    if currentUserFeedback == 0:
        ...
        if computerSuccessYesOrNo[0].lower() == "y":
            ...
            exit()
        dictionarySectionSize = len(dictionary)
        for x in range(5):
            if currentCompGuess[x] == "a":
                aScore *= 2.5
            elif currentCompGuess[x] == "b":
