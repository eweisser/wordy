...
def userTakesAGuess():
    ...
    for z in range(len(dictionary)):
        if currentUserGuess == dictionary[z]:
            found = True
    if found == False:
        if currentUserGuess == "q":
            ...
            print ("Sorry, that's not in my dictionary. Try again.")
            userTakesAGuess()
    else:
        for x in range(5):
            for y in range(5):
                if currentUserGuess[x] == compSecWord[y]:
...
def compTakesAGuess():
    ...
    global yScore
    global zScore
    # currentCompGuess = dictionary[math.floor(random()*len(dictionary))] # comp picks a random word
    candidateList = []
    candidateScoreList = []
    for z in range(50): # don't look at every word in the dictionary, just get 50
        candidateCompGuess = dictionary[math.floor(random()*len(dictionary))]
        candidateList.append(candidateCompGuess)
        candidateScore = 0
        if "a" in candidateCompGuess:
            candidateScore += aScore
        if "b" in candidateCompGuess:
            candidateScore += bScore
        if "c" in candidateCompGuess:
            candidateScore += cScore
        if "d" in candidateCompGuess:
            candidateScore += dScore
        if "e" in candidateCompGuess:
            candidateScore += eScore
        if "f" in candidateCompGuess:
            candidateScore += fScore
        if "g" in candidateCompGuess:
            candidateScore += gScore
        if "h" in candidateCompGuess:
            candidateScore += hScore
        if "i" in candidateCompGuess:
            candidateScore += iScore
        if "j" in candidateCompGuess:
            candidateScore += jScore
        if "k" in candidateCompGuess:
            candidateScore += kScore
        if "l" in candidateCompGuess:
            candidateScore += lScore
        if "m" in candidateCompGuess:
            candidateScore += mScore
        if "n" in candidateCompGuess:
            candidateScore += nScore
        if "o" in candidateCompGuess:
            candidateScore += oScore
        if "p" in candidateCompGuess:
            candidateScore += pScore
        if "q" in candidateCompGuess:
            candidateScore += qScore
        if "r" in candidateCompGuess:
            candidateScore += rScore
        if "s" in candidateCompGuess:
            candidateScore += sScore
        if "t" in candidateCompGuess:
            candidateScore += tScore
        if "u" in candidateCompGuess:
            candidateScore += uScore
        if "v" in candidateCompGuess:
            candidateScore += vScore
        if "w" in candidateCompGuess:
            candidateScore += wScore
        if "x" in candidateCompGuess:
            candidateScore += xScore
        if "y" in candidateCompGuess:
            candidateScore += yScore
        if "z" in candidateCompGuess:
            candidateScore += zScore
        # print (candidateCompGuess + " = " + str(candidateScore))
        candidateScoreList.append(candidateScore)

    # currentCompGuess = dictionary[math.floor(random()*len(dictionary))]
    currentCompGuess = candidateList[candidateScoreList.index(max(candidateScoreList))]

    print ("My guess is '" + currentCompGuess + "'. How many letters do I have right?")
    currentUserFeedback = input()
    currentUserFeedback = int(currentUserFeedback) # If the word matches 0 letters, each letter's score is set to 1.
    if currentUserFeedback == 0:					# If the word matches 1 letter, each letter's score increases by 2.
        for x in range(5):							# If the word matches 2 letters, each letter's score increases by 4.
            if currentCompGuess[x] == "a":			# If the word matches 3 letters, each letter's score increases by 10.
                aScore = 1							# If the word matches 4 letters, each letter's score increases by 50.
            elif currentCompGuess[x] == "b":		# If the word matches 5 letters, each letter's score increases by 1000.
                bScore = 1
