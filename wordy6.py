...
from random import random # There are len(dictionary) (5752 last time I checked) words in the dictionary. They'll be #0 to #5751
import math
...
def compTakesAGuess():
    ...
    if random() < 0.0000001 and turnNumber > 9:     # A very small chance to pick a random word; basically a placeholder in case I want to increase the odds later
        ...
        candidateHasAZeroList = []
        for z in range(1000): # don't look at every word in the dictionary, just get 1000
            candidateCompGuess = dictionary[math.floor(random()*len(dictionary))]
            candidateList.append(candidateCompGuess)    # this is a list of all the words the computer MIGHT guess this round; it'll   compare all of them
            candidateMultiplyScore = 1
            candidateAddScore = 0
            ...
            # candidateMultiplyScoreList[candidateAddScoreList.index(max(candidateAddScoreList))]:

        #if random() < turnNumber / 60:	# more likely to choose maximum-scoring word as time goes by (by turn 30, 50%)
        currentCompGuess = candidateList[candidateMultiplyScoreList.index(max(candidateMultiplyScoreList))]
        #else:      # this is supposed to be the eliminated letter part
        duplicateCandidateList = candidateList
        duplicateAddScoreList = candidateAddScoreList
        duplicateMultiplyScoreList = candidateMultiplyScoreList
        #while candidateMultiplyScoreList[candidateAddScoreList.index(max(candidateAddScoreList))] != 0:
        ...
        while duplicateMultiplyScoreList[duplicateAddScoreList.index(max(duplicateAddScoreList))] != 0:
            duplicateCandidateList.remove(duplicateCandidateList[maxAddIndex])
            duplicateAddScoreList.pop(maxAddIndex)
            duplicateMultiplyScoreList.pop(maxAddIndex)
            if len(duplicateCandidateList) == 0:
                break
            maxAddIndex = duplicateAddScoreList.index(max(duplicateAddScoreList))
        
        #print ("*********************")      
        #print ("Candidate list: " + str(duplicateCandidateList))
        #print ("Additive score list: " + str(duplicateAddScoreList))
        #print ("Multiplicative score list: " + str(duplicateMultiplyScoreList))
        #print ("High scoring word is " + duplicateCandidateList[maxAddIndex])
        #print ("...with additive score of " + str(duplicateAddScoreList[maxAddIndex]))
        #print ("...and multiplicative score of " + str(duplicateMultiplyScoreList[maxAddIndex]))
        #duplicateCandidateList.remove(duplicateCandidateList[maxAddIndex])
        #duplicateAddScoreList.pop(maxAddIndex)
        #duplicateMultiplyScoreList.pop(maxAddIndex)
        #maxAddIndex = duplicateAddScoreList.index(max(duplicateAddScoreList))
        ...
        #print ("*********************")
                ...
                #candidateHasAZeroList.append(candidateCompGuess)
        #while currentCompGuess in alreadyGuessedList:   # making sure we don't guess any words twice
